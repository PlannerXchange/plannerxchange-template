...
- prefer PX SDK `listAppData`, `getAppDataRecord`, `createAppDataRecord`, `updateAppDataRecord`, and `deleteAppDataRecord`; direct app-data adapters, routes, methods, request bodies, response projections, server-ID flow, type aliases, and spreads must remain statically resolvable
- deliberately dynamic or opaque executable app-data operations receive the blocking `app-data-request-contract-invalid` finding with SDK-first remediation; only a true PlannerXchange collector, analyzer execution, evidence, or provider failure is platform-owned
- dynamic creates must be replaced with literal or statically typed `POST /app-data` bodies containing required `recordType`, `status`, `schemaVersion`, and object `payload`; dynamic updates must use a statically typed `PATCH /app-data/{recordId}` body and a server-issued ID
...
- wrappers are supported when review can statically trace relative imports or uniquely resolved configured `tsconfig`/`jsconfig` path aliases, parameter forwarding, assignments, and aliases from the user action to the matching runtime helper and declaration ID; direct calls remain the recommended simplest pattern
- exports are not file ingress: Blob/download behavior, spreadsheet write APIs, and `.xlsx` filenames must not be treated as parsing without a file input, reader, drop/upload path, or parser call
...
- If the builder already stated what they want the app to do, map that outcome to the internal goal without asking again. Otherwise ask with plain choices: try it privately; list it for other advisors; let anyone try a sample using made-up information; show a public information page; match each firm's branding; use PlannerXchange information; or check every app outcome. Do not show internal goal IDs or platform jargon unless the builder asks.
