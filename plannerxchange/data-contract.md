...
- a local wrapper is supported when review can statically trace the import route or primary action through relative imports or uniquely resolved configured `tsconfig`/`jsconfig` path aliases, parameter forwarding, assignments, and stored aliases to the runtime helper and the same declaration ID. Ambiguous or unresolved local aliases remain unverified. A direct `ctx.openDataImportSession(...)` call remains the simplest recommended pattern.
