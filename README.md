...
- canonical import and canonical consumption are separate contracts. If an app imports and then presents a canonical category, it must also request the exact read permission, declare the category/read scope, perform a reachable PX read, retain canonical IDs, store only overlays in app-data, and commit the rebuilt artifact/provenance. A free-text Client, Household, or Account field backed by a local ID does not select PX data and blocks Data approval until integrated or honestly relabeled as app-local.
...
17. If Demo mode represents canonical data, use the starter's `createPlannerXchangeDemoDataClient` against the public `/canonical-demo` API, pin a literal scenario and `px_canonical_demo_v1`, and declare every exact object/field use in `canonicalDataUsageDeclarations`. Custom fields are excluded; do not install a Demo package or request a tarball.
...
21. Do not build or wait for a first-launch PX data-selection screen. PlannerXchange obtains administrator approval during installation and manages later restrictions. On normal launch, query approved canonical APIs, show human-readable labels, retain returned IDs, and refetch after canonical writes.
