...
- CSV/file imports must be declared; high-risk client/account/custodian CSVs must use declared PX import sessions (`px_import_session` + `ctx.openDataImportSession`), not app-owned parsers, import routes, or parent-matching logic. `openDataImportSession({ declarationId, mode: "canonical_store" })` returns only `{ mode: "canonical_store", status: "launched" }`; do not wait for `completed`, `completed_with_errors`, `cancelled`, `importJobId`, `canonicalRefs`, or `mappingSummary`. If TypeScript says `openDataImportSession` is missing, refresh `src/plannerxchange.ts` from the current template before changing the app architecture.
...
Before watching review feedback, the AI agent should ask the builder which goal applies now:
